...
/*认证页面切换*/
$('li').click(function(e) {
	var srcchange=$(this).find('.open_close').children().attr('src')
	if(srcchange.indexOf("renzheng_op.png") > 0 ){
		$(this).find('.open_close').children().attr('src','images/renzheng_c.png')
	}else{
		$(this).find('.open_close').children().attr('src','images/renzheng_op.png')
	}
	$(this).find('.false_diff').toggle()
});